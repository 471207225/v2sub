...
def openiptableRedirect():
    ...
    try:
        subprocess.call("iptables -t nat -A V2RAY -p tcp --match multiport ! --dports 12345,7890,22 -j REDIRECT --to-ports 12345",shell=True, stdout = subprocess.DEVNULL)
    except BaseException:
        print('以存在相应规则!跳过!')
...
if mode == 'changeNode':
    ...
    for i in range(len(serverListLink)):
        ...
        try:
            time.sleep(5)
            output = subprocess.check_output('curl -o /dev/null -s -w %{speed_download} -x socks5://127.0.0.1:7890 ' + testFileUrl, shell=True)
        except KeyboardInterrupt:
            break
        except BaseException:
            output = b'0.000'
